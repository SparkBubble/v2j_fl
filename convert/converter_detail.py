...
from convert.v2j_yacc_detail import parser
    ...
    def _translate_assignment(self):
        for var, lb, rb, expr in self.parser.sorted_assignments:
            try:
                ...
                verilog_code += f' = {expr}'
                javacode_list = parser.parse(verilog_code)
                self.assignments_javacode.extend(javacode_list)
            except Exception as e:
                print(f'Error in assignment statement: {expr}')
                raise e

    def toJava(self, dir_path:str, package_name:str='module'):
        """
