...
def p_ternary(p):
    ...
    code.append(f'    {name} = {p[3]["name"]};')
    code.append(f'else')
    code.append(f'    {name} = {p[5]["name"]};')
    p[0] = {
        'name': name,
        'code': code
    }
...
def p_logic_op(p):
    ...
    if p[2] == '&&':
        ...
        code.append(f'wire {name} = {p[1]["name"]}.logicalOr({p[3]["name"]});')
    p[0] = {
        'name': name,
        'code': code
    }
...
def p_bit_op(p):
    ...
    if p[2] == '&':
        ...
        code.append(f'wire {name} = {p[1]["name"]}.bitXor({p[3]["name"]});')
    elif p[2] == '|':
        code.append(f'wire {name} = {p[1]["name"]}.bitOr({p[3]["name"]});')
    p[0] = {
        'name': name,
        'code': code
    }
...
def p_relational_op(p):
    ...
    if p[2] == '<':
        ...
        code.append(f'wire {name} = {p[1]["name"]}.lessEqual({p[3]["name"]});')
    elif p[2] == '>':
        code.append(f'wire {name} = {p[1]["name"]}.greaterThan({p[3]["name"]});')
    elif p[2] == '>=':
        code.append(f'wire {name} = {p[1]["name"]}.greaterEqual({p[3]["name"]});')
    elif p[2] == '==':
        code.append(f'wire {name} = {p[1]["name"]}.isEqual({p[3]["name"]});')
    elif p[2] == '!=':
        code.append(f'wire {name} = {p[1]["name"]}.notEqual({p[3]["name"]});')
    p[0] = {
        'name': name,
        'code': code
    }
...
def p_shift_op(p):
    ...
    if p[2] == '<<':
        ...
        code.append(f'wire {name} = {p[1]["name"]}.shiftRight({p[3]["name"]});')
    p[0] = {
        'name': name,
        'code': code
    }
...
def p_arithmetic_op(p):
    ...
    if p[2] == '+':
        ...
        code.append(f'wire {name} = {p[1]["name"]}.sub({p[3]["name"]});')
    elif p[2] == '*':
        code.append(f'wire {name} = {p[1]["name"]}.mul({p[3]["name"]});')
    p[0] = {
        'name': name,
        'code': code
    }
...
def p_unary_op(p):
    ...
    if p[1] == '!':
        ...
        code.append(f'wire {name} = {p[2]["name"]}.bitNot();')
    elif p[1] == '&':
        code.append(f'wire {name} = {p[2]["name"]}.reduceAnd();')
    elif p[1] == '^':
        code.append(f'wire {name} = {p[2]["name"]}.reduceXor();')
    elif p[1] == '|':
        code.append(f'wire {name} = {p[2]["name"]}.reduceOr();')
    p[0] = {
        'name': name,
        'code': code
    }
...
def p_expr_num(p):
    ...
    p[0] = {
        'name': name,
        'code': [f'wire {name} = new wire({p[1][0] - 1}, 0, {p[1][1]});']
    }
