...
class VerilogDivider:
    ...
    def _translate_assignment(self):
        for var, lb, rb, expr in self.parser.sorted_assignments:
            try:
                ...
                verilog_code += f' = {expr}'
                divided_list = parser.parse(verilog_code)
                self.new_assignments.extend(divided_list)
    ...
    output [{{ port.highBit }}:{{ port.lowBit }}] {{ port.name }}{% if not loop.last %}, {% endif %}{% endfor %}
);

    {% for var in sorted_variables %}
    wire {{ var.name }}[{{ var.highBit }}: {{ var.lowBit }}];{% endfor %}

    // assignments{% for assign in assignments %}
    {{ assign }}{% endfor %}
